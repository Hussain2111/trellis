import { scrapedPostSchema, scrapedProfileSchema, type ScrapedPost, type ScrapedProfile } from '../providers/scraper/types';
...
export function normalizePostType(row: Row): ScrapedPost['type'] {
  const raw = String(pick(row, ['type', 'productType', 'product_type', '__typename']) ?? '')
    .toLowerCase();
